...
OTA and HTTP updates from GitHub. The device checks a manifest on GitHub Pages for stable and beta builds. The 10-inch model uses `https://peterhing.github.io/espframe/firmware/manifest.json` for stable firmware and `https://peterhing.github.io/espframe/firmware/beta/manifest.json` for beta firmware. Controls: device web UI at `http://<device-ip>/` under **Firmware** (and in Home Assistant).
